...
function onSaveRollHandler(rSource, rTarget, rRoll)
    ...
    if bAct and nodeEffect then
        ...
        if rRoll.sSaveDesc:match("%[REMOVE ON SAVE]") then
            BCEManager.modifyEffect(sPath, "Remove");
        elseif rRoll.sSaveDesc:match("%[DISABLE ON SAVE]") then
            BCEManager.modifyEffect(sPath, "Deactivate");
        end
...
function saveEffect(rTarget, rEffectComp)
    ...
    if sAbility and sAbility ~= "" then
        ...
        rSaveVsRoll.sDesc = rSaveVsRoll.sDesc .. " [" .. StringManager.capitalize(aParsedRemiander[1]) .. " DC " ..
                                rSaveVsRoll.nMod .. "]";
...
function replaceSaveDC(rNewEffect, rActor)
    ...
    if rNewEffect.sName:match("%[SDC]") and
        (rNewEffect.sName:match("SAVEE%s*:") or rNewEffect.sName:match("SAVES%s*:") or
            rNewEffect.sName:match("SAVEA%s*:") or rNewEffect.sName:match("SAVEONDMG%s*:")) then
