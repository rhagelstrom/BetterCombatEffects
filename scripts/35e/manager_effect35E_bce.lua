...
function customGetEffectsByType(rActor, sEffectType, aFilter, rFilterActor, bTargetedOnly) -- luacheck: ignore (cyclomatic complexity)
	...
	for _,v in pairs(aEffects) do
		...
		if (not bAdvancedEffects and (nActive ~= 0 or bActive)) or (bAdvancedEffects and (bActive or AdvancedEffects.isValidCheckEffect(rActor, v))) then
			-- Check targeting
				...
				for kEffectComp, sEffectComp in ipairs(aEffectComps) do
					...
					elseif rEffectComp.type == 'IFT' then
						...
						if comp_match then
							...
							nMatch = kEffectComp
							if nActive == 1 or bActive then table.insert(results, rEffectComp) end
						end
					end
...
function customHasEffect(rActor, sEffect, rTarget, bTargetedOnly, bIgnoreEffectTargets)
	...
	for _,v in pairs(aEffects) do
		...
		if (not bAdvancedEffects and (bActive or nActive ~= 0)) or (bAdvancedEffects and AdvancedEffects.isValidCheckEffect(rActor, v)) then
