...
function effectDeleted(nodeEffect)
    ...
    local tSearchEffect = BinarySearchManager.constructSearch(BCEManager.getEffectName(
        DB.getValue(nodeEffect, "label", "")), "remove", nodeEffect.getPath());
    tSearchEffect = BinarySearchManager.binarySearch(tGlobalEffects, tSearchEffect, 1, #tGlobalEffects);
    if not tSearchEffect then
...
function effectUpdated(nodeLabel)
    ...
    local tSearchEffect = BinarySearchManager.constructSearch(BCEManager.getEffectName(tEffectsLookup[sPath]), "update",
        sPath);
    tSearchEffect = BinarySearchManager.binarySearch(tGlobalEffects, tSearchEffect, 1, #tGlobalEffects);
    if not tSearchEffect then
...
function initGlobalEffects()
    for _, nodeEffect in pairs(DB.getChildrenGlobal("effects")) do
        local tSearchEffect = BinarySearchManager.constructSearch(
            BCEManager.getEffectName(DB.getValue(nodeEffect, "label", "")), "insert", DB.getPath(nodeEffect));
